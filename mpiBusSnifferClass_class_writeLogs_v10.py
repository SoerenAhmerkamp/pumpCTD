...
class mpiBusSniffer(Thread):
	...
	def clear(self):
		...
		self.data = ["No Data","No Data",0,"No Data","No Data","No Data","No Data","No Data","No Data","No Data"]
		self.winchSpeed = 0.5
...
#			self.insInterp = "Status check"
	...
	def interpTime(self,command):
		if self.insReq[0] == "\x03":
			#self.insInterp = len(self.insReq)
			outputTemp = struct.unpack('<7B',self.insReq[1:8])
			#print outputTemp
			day = outputTemp[0]
			month = outputTemp[1]
			year = outputTemp[2]
			...
			min = outputTemp[5]
			sec = outputTemp[6]
			self.data[8] = str(month) + "/" + str(day) + "/" + str(year) + " - " + str(hr) + ":" + str(min) + ":" + str(sec)
			#self.updateLists("Time",8)
	...
	def interpAnalog(self,command):
		...
		# Zyklops	
		if self.insAdd == 30 and channel == "\x00":	
			#self.insInterp = channel
			self.data[5] = round(self.insInterp*-0.0001,1)
			self.updateLists("Chlorophyll",5)
		#if self.insAdd == 31 and channel == "\x01":

		# Par
		if self.insAdd == 29 and channel == "\x00":
			self.data[9] = round(self.insInterp*0.01,1)
#			self.updateLists("Par",10)

	def interpAanderaa(self,command):
	    #self.fHFastCAT.write(datetime.datetime.now().strftime("%Y-%m-%d\t%H:%M:%S.%f\t"))
	    #self.fHFastCAT.write("%d \n" % int(command))
		command = self.insReq[0]
		#channel = self.insReq[1]
		
		outputTemp = struct.unpack('<H',self.insReq[1:3])
		
		#self.data[0] = self.insReq[0]
		
		self.insInterp = round(outputTemp[0]*0.01)
		
		self.data[4] = self.insInterp
		#self.updateLists("Par",8)
	...
	def interpFiresting(self,command):
		...
		if leng > 25 and self.insAdd == 42:
			...
			output = outputTemp[6:]
			self.BUSY = 1
			self.data[6] = round(float(output[1])/1000,1)
		...
		if leng > 25 and self.insAdd == 41:
			...
			output = outputTemp[6:]
			self.BUSY = 1
			self.data[4] = round(float(output[1])/1000,1)
			self.insInterp = output
