...
#			self.insInterp = "Status check"
	...
	def interpTime(self,command):
		if self.insReq[0] == "\x03":
			...
			min = outputTemp[5]
			sec = outputTemp[6]
			self.data[9] = str(hr) + ":" + str(min) + ":" + str(sec)
	...
	def interpAnalog(self,command):
		...
		if self.insAdd == 31 and channel == "\x00":	
			...
			self.lis[self.lisName["Chlorophyll"]].extend([self.data[5]])
			self.lisDepth[self.lisName["Chlorophyll"]].extend([self.data[2]])
				
		# Par
		if self.insAdd == 33 and channel == "\x00":
			self.data[8] = self.insInterp
